...
res.aov <- anova_test(data = soil2, dv = soil_moisture, wid = Temperature, within = Date)
...
#below is the model
summary(height <- lme(Height_cm~Species*Temperature, #typical model with percent herbivory as a dependent variable and species and warming as explanatory variables
                               data=height2, #what is the dataframe
                               random=~1|Plant_number, #random statement for which raised bed the plant came from
                               correlation=corAR1(form=~Date|Plant_number), #autoregressive correlation structure (good for height, which can only increase through time), saying that plants are nested within beds, which were sampled multiple dates
...
#this gives you the mean and standard errors for each treatment
emmeans(insectherbModel, pairwise~as.factor(warming), adjust="tukey")




#same model with a sqrt transformation to make the data normal
summary(insectherbModel <- lme(sqrt(avg_perc_herbivory)~as.factor(species)*as.factor(warming), #note how avg_perc_herbivory is square root transformed in this one
                               data=herbivoryData, 
                               random=~1|bed, 
                               correlation=corAR1(form=~date|bed/plant),
                               control=lmeControl(returnObject=T)))
check_model(insectherbModel)
anova.lme(insectherbModel, type='sequential') 
back.emmeans(emmeans(insectherbModel, pairwise~as.factor(warming), adjust="tukey"), transform='sqrt') #back.emmeans will backtransform the values for you ######## repeated measures anova on plant height ###########

soil_mod <- lme(soil_moisture ~ Temperature, 
                               data=soil2, 
                               random=~1|Plant_number,
                               correlation=corAR1(form = ~Date | Plant_number), 
                               control=lmeControl(returnObject=T))
