---
title: "Analysis 2.0"
author: "AMH"
date: "8/12/2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```


Amy's packages and working directory
```{r}
library(tidyverse)
library(ggpubr)
library(RColorBrewer)
library(rcompanion)

setwd("C:/Users/hrusk/Dropbox (Smithsonian)/2021_SummerInterns/AM")

dat <- read.csv("Heat_herbivory_alldata.csv")

```


Annika just messin' around 
```{r}
shawty <- 3*7*16
print(shawty)
```

Annika's packages and working directory
```{r}
setwd("/Users/annikamunson/Box Sync/Git SERC/AM-SERC-2021")

dat <- read.csv("Heat on Herbivory - All data (1).csv")

```

Data wrangling/cleaning
```{r}
#test height_cm is numeric and set to numeric#
is.numeric(height$Height_cm)
height$Height_cm <- as.numeric(height$Height_cm)

#create dataset for height graphing and analysis#
height <- dat %>%
  select(Date, Species, Temperature, Plant_number, Height_cm)

#height last day#
height2 <- height %>%
  filter(Date == "9-Aug")

#height first day#
height3 <- height %>%
  filter(Date == "8-Jul")

#merge height of last and first date and create a new column for change in height#
height4 <- merge(height2, height3, by = "Plant_number")

height4$delta_height <- height4 %>%
  mutate(height4$Height_cm.x - height4$Height_cm.y)






```

BarStats Function
```{r}

barGraphStats <- function(data, variable, byFactorNames) {
  count <- length(byFactorNames)
  N <- aggregate(data[[variable]], data[byFactorNames], FUN=length)
  names(N)[1:count] <- byFactorNames
  names(N) <- sub("^x$", "N", names(N))
  mean <- aggregate(data[[variable]], data[byFactorNames], FUN=mean)
  names(mean)[1:count] <- byFactorNames
  names(mean) <- sub("^x$", "mean", names(mean))
  sd <- aggregate(data[[variable]], data[byFactorNames], FUN=sd)
  names(sd)[1:count] <- byFactorNames
  names(sd) <- sub("^x$", "sd", names(sd))
  preSummaryStats <- merge(N, mean, by=byFactorNames)
  finalSummaryStats <- merge(preSummaryStats, sd, by=byFactorNames)
  finalSummaryStats$se <- finalSummaryStats$sd / sqrt(finalSummaryStats$N)
  return(finalSummaryStats)
  
}
```


```{r}
##Creating colourblind palette###
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")

##saved graph as variable to edit theme below##
g1 <- ggplot(data = barGraphStats(data = height, variable = "Height_cm", byFactorNames = c("Species", "Temperature")), aes(x=Species, y=mean, fill = Temperature)) +
    geom_bar(stat='identity', position=position_dodge()) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.2, position = position_dodge(0.9)) +
    xlab("Plant Species") + ylab("Plant Height (cm)") +
    scale_fill_manual(values = cbPalette) 

##setting theme to remove background grid and add axis lines##    
g1 + theme(panel.background = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), text = element_text(size = 16),axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))


```
Height on final day graph
```{r}
##saved graph as variable to edit theme below##
g1 <- ggplot(data = barGraphStats(data = height2, variable = "Height_cm", byFactorNames = c("Species", "Temperature")), aes(x=Species, y=mean, fill = Temperature)) +
    geom_bar(stat='identity', position=position_dodge()) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width = 0.2, position = position_dodge(0.9)) +
    xlab("Plant Species") + ylab("Plant Height (cm)") +
    scale_fill_manual(values = cbPalette) 

##setting theme to remove background grid and add axis lines##    
g1 + theme(panel.background = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"), text = element_text(size = 16),axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)), axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))
```
